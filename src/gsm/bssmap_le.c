...
static int osmo_bssmap_le_ie_dec_gad(struct gad_pdu *gad,
				     enum bssmap_le_msgt msgt, enum bssmap_le_iei iei,
				     struct osmo_bssmap_le_err **err, void *err_ctx,
				     const uint8_t *elem, uint8_t len)
{
	struct osmo_gad_err *gad_err;
	if (osmo_gad_dec(gad, &gad_err, err_ctx, elem, len)) {
...
enum bssmap_le_msgt bssmap_le_msgt(const uint8_t *data, uint8_t len)
{
